  ...
  <q-page class="q-pa-md">
    <q-card class="mt-2 q-pa-md">
      <q-card-section class="q-d-flex row q-flex-grow-1">
        <div class="text-h6 col">{{ label }} Promotion</div>
      ...
      <q-card-section>
        ...
        <label class="text-weight-medium q-pb-md">Campaign Name</label>
        <q-input
        ...
        <label class="text-weight-medium q-pb-md">Discount</label>
        <q-input
          v-model="promotion.discount_percentage"
          outlined
          type="number"
          dense
          class="q-mb-md"
        ...
        <div class="q-d-flex q-flex-grow-1 row">
          <div class="col q-px-sm">
            <label class="text-weight-medium q-pb-md">Start Date</label>
            <q-input
              filled
          ...
          <div class="col q-px-sm">
            <label class="text-weight-medium q-pb-md">End Date</label>
            <q-input
              filled
        ...
        <label class="text-weight-medium q-pb-md">Products</label>
        <q-select
          outlined
          v-model="promotion.products"
          :options="productList"
          option-label="name"
          option-value="id"
          emit-value
...
export class CreateEditPromotions extends Vue {
  ...
  loadProducts() {
    ProductsService.productsList("", 1, 1000).then((res: any) => {
  ...
  saveData() {
    if (this.promotion.id) {
      this.loading = true;
      CampaignsService.campaignsUpdate(this.promotion.id, this.promotion)
        .then((result: Promotion) => {
          ...
          this.$router.push(`/pages-admin/promotions/${result.id}/edit`);
        })
        .catch(() => {
          this.$q.notify({
      ...
      CampaignsService.campaignsCreate(this.promotion)
        .then((res: Promotion) => {
          this.$q.notify({
  ...
  loadData() {
    this.$q.loading.show();
    CampaignsService.campaignsRead(this.promotionId)
  ...
  deleteItem(item: Promotion) {
    if (item.id) {
      CampaignsService.campaignsDelete(item.id).then(() => {
        this.routeHome();
      });
    }
  }
  ...
  routeHome() {
    (this as any).$router.push("/pages-admin/promotions/");
  }
}
export default toNative(CreateEditPromotions);
</script>
